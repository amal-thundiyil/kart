...
#         path = pathlib.Path(prefix) / "help" / f'{ctx.command_path.replace(" ", "-")}.1'
#     else:
#         path = pathlib.Path(prefix) / "help" / f'{ctx.command_path.replace(" ", "-")}'
#     path.write_text()

#     help_render = get_renderer()
#     output = help_render._convert_doc_content(ctx).decode("utf-8")
